...
def meta(p,digest=False):
    ...
    r={'path':str(p),'name':p.name,'extension':ext,'mime':mimetypes.guess_type(p.name)[0],'size_bytes':s.st_size,'created_at':datetime.fromtimestamp(s.st_ctime).isoformat(),'modified_at':datetime.fromtimestamp(s.st_mtime).isoformat(),'is_image':ext in IMAGE_EXT}
...
def extract(p):
    ...
    if ext=='.docx':
        ...
        d=base.Document(str(p));out=[x.text for x in d.paragraphs];out += [' | '.join(c.text for c in row.cells) for t in d.tables for row in t.rows];return '\n'.join(out)
    if ext in ('.xlsx','.xlsm'):
...
async def scan(body:dict):
    try:
        root=Path(body.get('folder','')).expanduser();limit=min(int(body.get('limit',5000)),20000);rows=[];total=0
        for i,p in enumerate(files(root)):
            if i>=limit:break
            try:r=meta(p,bool(body.get('with_hash')));rows.append(r);total+=r['size_bytes']
...
async def extract_file(body:dict):
    try:
        p=Path(body.get('path','')).expanduser();text=extract(p);limit=min(int(body.get('max_chars',200000)),500000);return {'ok':True,'metadata':meta(p),'text':text[:limit],'characters':len(text),'truncated':len(text)>limit,'read_only':True}
    except Exception as e:return JSONResponse({'ok':False,'error':str(e)},400)
...
async def context(body:dict):
    ...
    for raw in body.get('paths',[]):
        ...
        try:p=Path(raw).expanduser();text=extract(p);take=min(len(text),budget-used);parts.append(f'===== SOURCE: {p} =====\n{text[:take]}');used+=take
...
async def duplicates(body:dict):
    try:
        ...
        for p in files(root):
            ...
            try:r=meta(p,True);groups.setdefault(r['sha256'],[]).append(r);count+=1
            except (OSError,PermissionError):pass
        proposals=[]
        for g in groups.values():
            if len(g)<2:continue
            keep=min(g,key=lambda x:(x['created_at'],x['path'].lower()));dup=[x for x in g if x['path']!=keep['path']]
            proposals.append({'keep':keep,'duplicates':dup,'recoverable_bytes':sum(x['size_bytes'] for x in dup),'reason':'Exact SHA-256 duplicate. Oldest creation timestamp proposed as canonical. No deletion performed.'})
        return {'ok':True,'folder':str(root),'images_scanned':count,'duplicate_groups':proposals,'group_count':len(proposals),'recoverable_bytes':sum(x['recoverable_bytes'] for x in proposals),'actions_performed':[],'read_only':True}
    except Exception as e:return JSONResponse({'ok':False,'error':str(e)},400)
@APP.get('/api/workspace/policy')
async def workspace_policy():return {'read_only_default':True,'delete_enabled':False,'move_enabled':False,'rename_enabled':False,'preserve_original_metadata':True,'preserve_originals':True,'description':'SS analyses files and proposes actions. It does not delete, rename, move or overwrite files.'}
