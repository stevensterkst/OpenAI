...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Label(root, text="PRIMARY: local source transcript + source-language summary. "
                   "English summary is only a translation. Full translation and analysis are optional.").pack(
            anchor="w", pady=(0, 14))
        ...
        self.combo(opts, "Source language", self.language,
                   ["auto", "ca", "es", "en", "fr", "de", "it", "nl", "pt", "pl"], 0, 0)
        self.combo(opts, "Whisper model", self.model,
                   ["tiny", "base", "small", "medium", "large-v3"], 1, 0)
        ...
        ttk.Label(opts, text="Comma-separated names, organisations or specialist terms to bias recognition.",
                  wraplength=520).grid(row=2, column=2, columnspan=3, sticky="w", padx=(28, 0), pady=(8, 3))

        ttk.Checkbutton(opts, text="Keep word-level timestamps", variable=self.word_timestamps).grid(
            row=3, column=0, columnspan=2, sticky="w", pady=3)
        ttk.Checkbutton(opts, text="Run source-grounded meeting/evidence analysis", variable=self.analysis).grid(
            row=4, column=0, columnspan=2, sticky="w", pady=3)
        ttk.Checkbutton(opts, text="Also translate the FULL source transcript",
                        variable=self.translate_transcript).grid(row=5, column=0, columnspan=2, sticky="w", pady=3)

        ttk.Label(opts, text="Target language").grid(row=5, column=2, sticky="w", padx=(28, 8))
        ttk.Entry(opts, textvariable=self.target, width=32).grid(row=5, column=3, sticky="w")
        ttk.Label(opts, text="Any language supported by the selected Ollama model.").grid(row=5, column=4, sticky="w")

        ttk.Label(opts, text="Cost").grid(row=6, column=2, sticky="w", padx=(28, 8))
        ttk.Label(opts, text="$0 / €0 paid API — this application does not call OpenAI APIs").grid(
            row=6, column=3, columnspan=2, sticky="w")

        ttk.Label(opts, text="Output").grid(row=7, column=0, sticky="w", pady=(8, 0))
        ttk.Entry(opts, textvariable=self.output).grid(row=7, column=1, columnspan=4, sticky="ew", pady=(8, 0))
