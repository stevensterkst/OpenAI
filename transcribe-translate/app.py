...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Label(opts,text="Comma-separated names, organisations or specialist terms to bias recognition.",wraplength=520).grid(row=3,column=2,columnspan=3,sticky="w",padx=(28,0),pady=(8,3))
        ttk.Checkbutton(opts,text="Keep word-level timestamps",variable=self.word_timestamps).grid(row=5,column=0,columnspan=2,sticky="w",pady=3)
        ...
        ttk.Label(opts,text="Analysis output language").grid(row=5,column=2,sticky="w",padx=(28,8))
        ttk.Entry(opts,textvariable=self.analysis_language,width=32).grid(row=5,column=3,sticky="w")
        ttk.Label(opts,text="source = original language; otherwise any Ollama-supported language.").grid(row=5,column=4,sticky="w")
        ttk.Label(opts,text="Search transcript").grid(row=6,column=0,sticky="w",pady=3)
        ttk.Entry(opts,textvariable=self.search_query,width=48).grid(row=9,column=1,sticky="w")
        ttk.Label(opts,text="Exact text search; results include timestamps. Counts/top terms are language-neutral.",wraplength=520).grid(row=6,column=2,columnspan=3,sticky="w",padx=(28,0))
        ...
        ttk.Label(opts,text="Transcript-grounded Q&A").grid(row=8,column=0,sticky="w",pady=3)
        ttk.Entry(opts,textvariable=self.qa_question,width=48).grid(row=7,column=1,sticky="w")
        ttk.Label(opts,text="Optional question answered only from this recording.").grid(row=8,column=2,columnspan=3,sticky="w",padx=(28,0))
        ...
        ttk.Checkbutton(opts,text="Also translate the FULL source transcript",variable=self.translate_transcript).grid(row=10,column=0,columnspan=2,sticky="w",pady=3)
        ttk.Label(opts,text="Target language").grid(row=11,column=2,sticky="w",padx=(28,8))
        ttk.Entry(opts,textvariable=self.target,width=32).grid(row=10,column=3,sticky="w")
        ttk.Label(opts,text="Any language supported by the selected Ollama model.").grid(row=10,column=4,sticky="w")
        ...
        ttk.Label(opts,text="Core processing: $0 / €0 paid API. Optional OpenAI queries are user-triggered and can incur API charges.").grid(row=11,column=3,columnspan=2,sticky="w")
        ttk.Checkbutton(opts,text="Keep source/downloaded media and extracted audio files (default OFF)",variable=self.keep_media).grid(row=12,column=0,columnspan=2,sticky="w",pady=(8,0))
        ttk.Label(opts,text="Default: only transcript/text outputs are retained.").grid(row=12,column=2,columnspan=3,sticky="w",pady=(8,0))
        ttk.Label(opts,text="Output").grid(row=13,column=0,sticky="w",pady=(8,0))
        ttk.Entry(opts,textvariable=self.output).grid(row=13,column=1,columnspan=4,sticky="ew",pady=(8,0)); opts.columnconfigure(3,weight=1)
