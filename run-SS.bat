...
if not defined REPO (echo ERROR: Could not locate the SS Git checkout. &echo Expected this BAT in the checkout or its parent folder. &pause&exit /b 1)
cd /d "%REPO%"
...
if not exist "%PY%" (where py >nul 2>nul || (echo ERROR: Python is required.&pause&exit /b 1) & py -m venv .venv || (echo ERROR: venv creation failed.&pause&exit /b 1))
